        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="mb-6 flex justify-between items-center">
                    <h1 class="text-3xl font-bold">efts</h1>
                    <div class="relative w-64">
                        <input
                            type="text"
                            placeholder="Search efts..."
                            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            :value="search"
                            @input="$inertia.get(route('efts.index'), { search: $event.target.value }, { preserveState: true })"
                        />
                        <button
                            @click="$inertia.get(route('efts.index'), { search })"
                ...
                <div class="border border-gray-800 bg-default text-white shadow-sm overflow-hidden shadow-sm sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="border border-gray-800 bg-default text-white shadow-sm divide-y divide-gray-200">
                            <tr v-for="stock in efts" :key="stock.symbol" class="">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <Link :href="route('efts.show', stock.symbol)" class="text-xl sm:text-2xl tex-white font-semibold font-medium">
                                        {{ stock.symbol }}
                                    </Link>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <tr v-if="efts.length === 0">
                                <td colspan="5" class="px-6 py-4 text-center text-gray-500">No efts found</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
